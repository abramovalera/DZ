...
public class COMANDTEST {
    ...
    static void selectorExamples() {
        ...
        $(".HeaderMenu-nav").find(byText("Solutions")).hover(); //если вы ищете элемент по классу, селектор должен начинаться с точки
    ...
    static void actionsExamples() {
        ...
        // Получаем объект Actions для выполнения сложных действий с мышью
        actions()
                // Перемещаем курсор к центру элемента <div>
                .moveToElement($("div"))
                // Нажимаем и удерживаем левую кнопку мыши на этом элементе
                .clickAndHold()
                // Перемещаем курсор (и, соответственно, элемент) на 300 пикселей вправо и 200 пикселей вниз
                .moveByOffset(300, 200)
                // Отпускаем кнопку мыши, завершая операцию перетаскивания
                .release()
                // Выполняем всю цепочку действий
                .perform();
